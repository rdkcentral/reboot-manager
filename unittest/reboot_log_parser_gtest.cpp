...
TEST_F(LogParserTest, update_reboot_info_PlatcoRequiresFlagsAlways) {
    EnvContext ctx;
    memset(&ctx, 0, sizeof(EnvContext));
    ctx.platcoSupport = true;

    system("rm -f /tmp/stt_received /tmp/rebootInfo_Updated /tmp/Update_rebootInfo_invoked");
...
TEST_F(LogParserTest, update_reboot_info_RequiresFlagsWithInvokedFlagPresent) {
    EnvContext ctx;
    memset(&ctx, 0, sizeof(EnvContext));
    ctx.platcoSupport = true;
